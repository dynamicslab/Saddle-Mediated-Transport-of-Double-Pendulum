"""
# This file is used to plot the tubes of double pendulum's L2 saddle points (Using experimental parameters), and generate a color wheel
# Coded By: KK
...
## Load the POData
@load "Results/EDP_L1PO_Eng_$(EngTarget)_Tube.jld2" p_nf_edp x_inc_L1 PO2_Time PO2_Data EngEq1 EngEq2 EngEq3 EngEq4
...
Inc_pos_unstable=ExtractIncFromEndPoint(TrajVarInc2_pos_unstable_NoFric) 
Inc_neg_stable=ExtractIncFromEndPoint(TrajVarInc2_neg_stable_NoFric) 
...
#Inc_pos_unstable=-StoredCutDataL1[2]
#Inc_neg_stable=zeros(size(Inc_pos_unstable))
#Inc_neg_stable[1,:]=-Inc_pos_unstable[1,:]
#Inc_neg_stable[2,:]=-Inc_pos_unstable[2,:]
#Inc_neg_stable[3,:]=Inc_pos_unstable[3,:]
#Inc_neg_stable[4,:]=Inc_pos_unstable[4,:]
...
ax1=Axis(TubePlot[1,1],xlabel="θ2",ylabel="dθ2")
Makie.scatter!(ax1,Inc_pos_unstable[2,:],Inc_pos_unstable[4,:],color=col_p,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,Inc_neg_stable[2,:],Inc_neg_stable[4,:],color=col_o,strokewidth=0,markersize=mks)
ax1.aspect=AxisAspect(1/1)
display(TubePlot)

## Once we made sure the tube Poincare cut is correct, let's start create a Polygon 
AxisNum1=2;AxisNum2=4
# Generate polygon
PolyPoinsX=vcat(Inc_pos_unstable[AxisNum1,:],Inc_pos_unstable[AxisNum1,1])
PolyPoinsY=vcat(Inc_pos_unstable[AxisNum2,:],Inc_pos_unstable[AxisNum2,1])
...
# PO2=[0,0,20.704284411720227,-20.03102661145768]
# PO2=[pi,0,3.9190069634750704,3.471236285823615]
...
Makie.scatter!(ax1,[0],[-20.03102661145768],markersize=14,color="black")
save("Figures/EDP_L2_ColorWheel_Sim0.png", PolyPlot,px_per_unit=100,pt_per_unit=100)
display(PolyPlot)

## Noww find out the intial condition using energy relation
IncSim=zeros(4,size(IntersectPoints,1))
angle_index=1
for i=1:size(IntersectPoints,1)
    IncSim[2,i]=IntersectPoints[i][1]
    IncSim[4,i]=IntersectPoints[i][2]
    IncSim[3,i]=DPE_Cal_dTheta(EngTarget,IncSim[:,i],p_nf_edp,angle_index;pos_or_neg=0)
end
...
##
PolyPlot=Scene()
Makie.scatter!(PolyPlot,Inc_pos_unstable[2,:],Inc_pos_unstable[3,:],Inc_pos_unstable[4,:],color=col_p,strokewidth=0,markersize=1*mks)
Makie.scatter!(PolyPlot,Inc_neg_stable[2,:],Inc_neg_stable[3,:],Inc_neg_stable[4,:],color=col_o,strokewidth=0,markersize=1*mks)
Makie.scatter!(PolyPlot,IncSim[[2,3,4],:],markersize=1*mks,color=ColorWheel)
Makie.xlabel!("theta_2")
...
cb_cut1=ContinuousCallback(cut_function1!,affect_cut!,save_positions = (true,false));

## Now simulate all those points 
Tsim=1.6;everystepsave=false;backward=0
Gap=1;tspan=(0.0,Tsim);
FinalSimInc=IncSim[:,1:Gap:end]

## Do not use EnsembleThreads() approach, which will crash the VS code
TrajInTube_End=SimulateManifold(FinalSimInc,p_nf_edp,backward,Tsim,everystepsave,cb_cut1;saveend=true,saveat=true,experimental=true)

## Plot those trajectory
lw1=5;lw2=5;lw3=5
mks=20;mk1=20;Transform=0
AxisNum1=1;AxisNum2=2;AxisNum3=4;ThreeD=0
Mod=0;color1=col_p;color2=col_o
gap_plot=100;lineplot=1
...
TrajPlot=MakiePlotMono(TrajPlot,PO2_Data,TrajInTube_End,color1,ThreeD,AxisNum1,AxisNum2,AxisNum3,Mod,Transform,mks,lineplot,lw1,gap=gap_plot)
#Makie.scatter!(TrajPlot,Inc_pos_unstable[2,:],Inc_pos_unstable[3,:],Inc_pos_unstable[4,:],color=col_p,strokewidth=0,markersize=1*mks)
#Makie.scatter!(TrajPlot,Inc_neg_stable[2,:],Inc_neg_stable[3,:],Inc_neg_stable[4,:],color=col_o,strokewidth=0,markersize=1*mks)
#Makie.scatter!(TrajPlot,IncSim[[2,3,4],:],markersize=1*mks,color=ColorWheel)
#Makie.scatter!(TrajPlot,Inc_pos_unstable[1,:],Inc_pos_unstable[2,:],color=col_p,strokewidth=0,markersize=1*mks)
#Makie.scatter!(TrajPlot,Inc_neg_stable[1,:],Inc_neg_stable[2,:],color=col_o,strokewidth=0,markersize=1*mks)
#Makie.scatter!(TrajPlot,IncSim[[1,2],:],markersize=1*mks,color=ColorWheel)
Makie.xlabel!("theta_1")
Makie.ylabel!("theta_2")
#Makie.zlabel!("dtheta_2")
#Makie.scale!(TrajPlot,1,0.1,0.1)
display(TrajPlot)

## Get the final position of each point
TrajInTube_Data=ExtractIncFromEndPoint(TrajInTube_End)
ColorWheel=ColorWheel[(TrajInTube_Data[1,:].<2*pi+0.01).*(TrajInTube_Data[1,:].>2*pi-0.01)]
TrajInTube_Data=TrajInTube_Data[:,(TrajInTube_Data[1,:].<2*pi+0.01).*(TrajInTube_Data[1,:].>2*pi-0.01)]

## Determine how many points lives inside the intersection
Sim1Points = vec(SVector.(TrajInTube_Data[2,:],TrajInTube_Data[4,:]))
...
Makie.scatter!(ax1,Inc_pos_unstable[2,:],Inc_pos_unstable[4,:],color=col_p,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,Inc_neg_stable[2,:],Inc_neg_stable[4,:],color=col_o,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,TrajInTube_Data[[2,4],:],markersize=14,color=ColorWheel)
#Makie.scatter!(ax1,Sim1PointsInside,markersize=14,color="red")
Makie.scatter!(ax1,[0],[-20.03102661145768],markersize=14,color="black")
save("Figures/EDP_L2_ColorWheel_Sim1.png",PolyPlot)
display(PolyPlot)

## Now simulate this again
TrajInTube_Data_NewInc=TrajInTube_Data
TrajInTube_Data_NewInc[1,:]=TrajInTube_Data_NewInc[1,:].-2*pi
...
TrajInTube_Data_2_Returned=TrajInTube_Data_2[:,(TrajInTube_Data_2[1,:].<2*pi+0.01).*(TrajInTube_Data_2[1,:].>2*pi-0.01)]
ColorWheel_Returned=ColorWheel[(TrajInTube_Data_2[1,:].<2*pi+0.01).*(TrajInTube_Data_2[1,:].>2*pi-0.01)]

## Determine how many points lives inside the intersection
Sim2Points = vec(SVector.(TrajInTube_Data_2_Returned[2,:],TrajInTube_Data_2_Returned[4,:]))
...
Makie.scatter!(ax1,Inc_pos_unstable[2,:],Inc_pos_unstable[4,:],color=col_p,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,Inc_neg_stable[2,:],Inc_neg_stable[4,:],color=col_o,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,TrajInTube_Data_2_Returned[[2,4],:],markersize=14,color=ColorWheel_Returned)
#Makie.scatter!(ax1,Sim2PointsInside,markersize=14,color="red")
Makie.scatter!(ax1,[0],[-20.03102661145768],markersize=14,color="black")
save("Figures/EDP_L1_ColorWheel_Sim2.png",PolyPlot)
display(PolyPlot)

## Now simulate again
TrajInTube_Data_3_NewInc=TrajInTube_Data_2_Returned
TrajInTube_Data_3_NewInc[1,:]=TrajInTube_Data_3_NewInc[1,:].-2*pi
...
TrajInTube_Data_3_Returned=TrajInTube_Data_3[:,(TrajInTube_Data_3[1,:].<2*pi+0.01).*(TrajInTube_Data_3[1,:].>2*pi-0.01)]
ColorWheel_Returned_3=ColorWheel_Returned[(TrajInTube_Data_3[1,:].<2*pi+0.01).*(TrajInTube_Data_3[1,:].>2*pi-0.01)]

## Determine how many points lives inside the intersection
Sim3Points = vec(SVector.(TrajInTube_Data_3_Returned[2,:],TrajInTube_Data_3_Returned[4,:]))
...
Makie.scatter!(ax1,Inc_pos_unstable[2,:],Inc_pos_unstable[4,:],color=col_p,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,Inc_neg_stable[2,:],Inc_neg_stable[4,:],color=col_o,strokewidth=0,markersize=mks)
Makie.scatter!(ax1,TrajInTube_Data_3_Returned[[2,4],:],markersize=14,color=ColorWheel_Returned_3)
Makie.scatter!(ax1,Sim3PointsInside,markersize=14,color="red")
Makie.scatter!(ax1,[0],[-20.03102661145768],markersize=14,color="black")
save("Figures/EDP_L2_ColorWheel_Sim3.png",PolyPlot)
display(PolyPlot)
...
cb1 = ContinuousCallback(conditionSaddle1,affect_cut!,nothing,save_positions = (true,true))
#x_inc_L2=[pi,0,3.9190069634750704,3.471236285823615]
x_inc_L2=[0,0,20.704284411720227,-20.03102661145768]
DP_Prob_Saddle1=ODEProblem(DPE_NoFric!,x_inc_L2,(0,2.0),p_nf_edp)
#
...
#
PO2_Time=Array(sol_saddle1.t)
PO2_Data=Array(sol_saddle1)

## Now plot this 2π periodic orbits, and save it
# Plot the positiveeps plot
PoincarPlot=Plots.plot(size=(650,400),tickfontsize=20)
PoincarPlot=Plots.plot!(PoincarPlot,PO2_Data[1,:],PO2_Data[3,:],color=col_p,
    label="",linewidth=2.5,framestyle=:box)
savefig(PoincarPlot,"Figures/EDP_2pi_PO2_1_vs_3.pdf") 

##
PoincarPlot=Plots.plot(size=(650,400),tickfontsize=20)
PoincarPlot=Plots.plot!(PoincarPlot,PO2_Data[2,:],PO2_Data[4,:],color=col_p,
    label="",linewidth=2.5,framestyle=:box)
savefig(PoincarPlot,"Figures/EDP_2pi_PO2_2_vs_4.pdf") 

##
PoincarPlot=Plots.plot(size=(650,400),tickfontsize=20,xticks = ([0,π,2π], ["0", "\\pi", "2\\pi"]),xlim=(-0.1,2*π+0.1))
PoincarPlot=Plots.plot!(PoincarPlot,PO2_Data[1,:],PO2_Data[2,:],color=col_p,
    label="",linewidth=2.5,framestyle=:box)
savefig(PoincarPlot,"Figures/EDP_2pi_PO2_1_vs_2.pdf") 
